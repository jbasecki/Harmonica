                ...
                <div style={{ position: 'relative', zIndex: 10, height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ width: '95%', maxWidth: isPreview ? '1100px' : '850px', textAlign: 'center' }}>
                                ...
                                <div style={{ position: 'absolute', bottom: '100px', left: '5%', right: '5%', display: 'flex', justifyContent: 'center', gap: '20px', flexWrap: 'wrap' }}>
                                    {selectedTiles.map((tile, idx) => (
                                        <div key={idx} style={{ display: 'flex', gap: '10px' }}>
                                            <img src={getLetterUrl(tile.charAt(0))} style={{ width: '130px', borderRadius: '15px', border: '4px solid gold', boxShadow: '0 0 15px gold' }} />
                                            <img src={getLetterUrl(tile.charAt(tile.length - 1))} style={{ width: '130px', borderRadius: '15px', border: '4px solid gold', boxShadow: '0 0 15px gold' }} />
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div style={{ marginTop: '20px', display: 'flex', gap: '25px', justifyContent: 'center' }}>
                            <button onClick={() => setIsPreview(!isPreview)} style={{ background: 'rgba(255,255,255,0.95)', padding: '15px 35px', borderRadius: '50px', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}>{isPreview ? '✍️' : '👁️'}</button>
                            <button onClick={handleSend} style={{ background: '#ff0000', color: '#fff', padding: '15px 60px', borderRadius: '50px', border: 'none', fontWeight: 'bold', fontSize: '1.4rem', cursor: 'pointer', boxShadow: '0 0 20px rgba(255,0,0,0.6)' }}>Send a Vibe</button>
                        </div>
                    </div>
                    ...
                    <div style={{ marginLeft: '50px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        <div style={{ background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(10px)', padding: '15px', borderRadius: '30px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px', border: '1px solid gold' }}>
                            {SCENES.map((s) => (
                                <button key={s.id} onClick={() => setSelectedScene(s)} style={{ width: '65px', height: '65px', borderRadius: '20px', border: selectedScene.id === s.id ? '3px solid gold' : '1px solid rgba(255,255,255,0.3)', background: selectedScene.id === s.id ? '#fff' : 'rgba(0,0,0,0.5)', color: selectedScene.id === s.id ? '#000' : '#fff', cursor: 'pointer', fontWeight: 'bold' }}>{s.label}</button>
                            ))}
                        </div>
                        <button onClick={() => setIsCleanView(true)} style={{ background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(10px)', border: '1px solid gold', borderRadius: '30px', padding: '25px', cursor: 'pointer', fontSize: '3rem', color: '#fff' }}>👁️</button>
                    </div>
                </div>
            )}
        </main>
